...
6. **Deploy gate — partially closed, pending first run on `main`.** Checked 2026-09-24: the repo is **private on a free GitHub plan**, so branch protection, rulesets and Environment required reviewers all return `403 Upgrade to GitHub Pro or make this repository public`. Chosen workaround (in-workflow gate, no cost): `ci.yml` now also runs on push to `main`; its `deploy-api` / `deploy-web` jobs call the deploy workflows (now `workflow_call` + `workflow_dispatch`, no push trigger) only when **both** `api` and `web` CI jobs pass **and** the `changes` job sees that component's files changed (`git diff github.event.before..sha`; first push or unknown `before` deploys both). Deploys use a per-component `concurrency` group so two never overlap. Change detection was tested locally against past commits. **What this does not do:** it cannot stop a direct push to `main`, cannot require a PR or a human approval, and cannot block a merge on red checks (it only stops a red build from being *deployed*). `workflow_dispatch` on the deploy workflows is an intentional manual override that skips CI. The real fix is GitHub Pro (branch protection + a `production` Environment with a reviewer) or a public repo; revisit before real user data lands.
